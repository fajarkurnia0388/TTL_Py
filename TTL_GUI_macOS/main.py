...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('TTL Tether Switcher (macOS GUI)')
    ...
    def refresh_status(self):
        ...
        if ttl is not None:
            if ttl == TTL_TETHER:
                ...
                self.label_status.setText(f'Status: Normal Mode (TTL=64)')
    ...
    def set_ttl(self, value):
        # Set TTL via sysctl (butuh sudo)
        try:
            result = subprocess.run(['sudo', 'sysctl', f'net.inet.ip.ttl={value}'], capture_output=True, text=True)
            if result.returncode == 0:
                QMessageBox.information(self, 'Sukses', f'TTL di-set ke {value}.')
            else:
                QMessageBox.critical(self, 'Error', result.stderr or 'Gagal set TTL.')
    ...
    def get_current_ttl(self):
        # Cek TTL dari sysctl
        try:
            result = subprocess.run(['sysctl', 'net.inet.ip.ttl'], capture_output=True, text=True)
            if result.returncode == 0:
                out = result.stdout.strip()
                if 'net.inet.ip.ttl:' in out:
...
def main():
    if platform.system() != 'Darwin':
        print('Aplikasi ini hanya untuk macOS.')
        return
